...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  $insertSQL = sprintf("UPDATE usuario SET nombre=%s, cedula=%s, id_dependencia=%s, solicitud_add=%s, solicitud_edit=%s, solicitud_del=%s, corres_add=%s, corres_edit=%s, solicitud_asociar=%s, corres_del=%s, corres_reporte=%s, corres_buscar=%s, dependencia_add=%s, dependencia_edit=%s, dependencia_del=%s, usuario_add=%s, usuario_edit=%s, usuario_del=%s, municipio_add=%s, municipio_edit=%s, municipio_del=%s, parroquia_add=%s, parroquia_edit=%s, parroquia_del=%s, config=%s, consulta=%s, correspondencia=%s WHERE id=%s",
...
mysql_select_db($database_conexion, $conexion);
$query_RS_U = "SELECT * FROM usuario WHERE id='$_GET[id]'";
$RS_U = mysql_query($query_RS_U, $conexion) or die(mysql_error());
$row_RS_U = mysql_fetch_assoc($RS_U);
$totalRows_RS_U = mysql_num_rows($RS_U);
?>
<html>
<head>
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
              <td colspan="2" class="Textonegro"><table width="100%" border="0" cellpadding="2" cellspacing="2" class="Textonegro">
                <tr>
                  ...
                  <td><input name="nombre" type="text" id="nombre" value="<?=$row_RS_U[nombre]?>"></td>
                  <td class="Negrita">Nro. C&eacute;dula: </td>
                  <td><input name="cedula" type="text" id="cedula" value="<?=$row_RS_U[cedula]?>" maxlength="8"></td>
                </tr>
                <tr>
                  <td class="Negrita">Dependencia:</td>
                  <td colspan="3">
				  <select name="id_dependencia" id="id_dependencia">
					  <option value="" <?php if (!(strcmp("", $row_RS_U['id_dependencia']))) {echo "selected=\"selected\"";} ?>>[Seleccione]</option>
					  <?php	do {  ?>
					  <option value="<?php echo $row_RS_D['id']?>" <?php if (!(strcmp($row_RS_D['id'], $row_RS_U['id_dependencia']))) {echo "selected=\"selected\"";} ?>><?php echo utf8_decode($row_RS_D['dependencia'])?></option>
					  <?php
            ...
            <tr>
			  ...
			  <? do{ ?>
				...
				<? } ?>
                <tr>
                  <td width="3%"><input <?php if (!(strcmp($row_RS_U[$row_RS_P[campo]],1))) {echo "checked=\"checked\"";} ?> type="checkbox" name="<?=$row_RS_P[campo]?>" value="1"></td>
            ...
            <tr>
              <td class="Textonegro"><input name="Submit" type="submit" class="BotonRojo" value="[ Guardar ]">
