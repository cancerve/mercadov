<?php
	...
	*/
	...
	for ($a=0;$a<$cantRSPedidoDetalle;$a++){
		...
		$AL_NombreSede 		= utf8_decode(strtoupper($objConexion->obtenerElemento($RSPedidoDetalle,$a,"AL_NombreSede")));
		$AL_NombreGerencia 	= $objConexion->obtenerElemento($RSPedidoDetalle,$a,"AL_NombreGerencia");
	
		///////////// NOMBRE SEDES ///////////////////
		if ($BI_Sede != $AL_NombreSede){
			if ($fila!=6){
		...
		if ($BI_Gerencia != $AL_NombreGerencia){
			$contarG++;	
		...
		$objPHPExcel->getActiveSheet()->SetCellValue($Columna2, '=SUM(B'.$fila.':'.$UltColumna2.')');
