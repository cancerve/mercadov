<?php 
	...
	$NU_IdDescuento = $_GET['NU_IdDescuento'];
	
	$RS 		= $objDescuentoN->listarDescuentoN($objConexion,$NU_IdDescuento);
	$cantRS 	= $objConexion->cantidadRegistros($RS);
?>
<?php
	
	if ($cantRS>0){
		$_SESSION['NU_IdDescuento'] = $objConexion->obtenerElemento($RS,0,"NU_IdDescuento");
		$FE_Registro = explode(" ",$objConexion->obtenerElemento($RS,0,"FE_Registro"));
		...
		$_SESSION['$anio'] = $mes[0];
		$_SESSION['mercado_NU_IdMercado'] = $objConexion->obtenerElemento($RS,0,"mercado_NU_IdMercado");
	}
...
class PDF extends FPDF{
	function Header(){
		...
		$this->Cell(0,0,'Consolidado de Descuento de Nomina Nro.: 0'.$_SESSION['NU_IdDescuento'],0,0,'R');		
		$this->Ln(10);
